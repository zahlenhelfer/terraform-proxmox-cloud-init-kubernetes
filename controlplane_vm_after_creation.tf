...
output "ctrl_ipv4_address" {
  ...
  value      = proxmox_virtual_environment_vm.k8s-ctrl[each.value.name].ipv4_addresses[1][0]
}

resource "local_file" "ctrl-ip" {
  content         = proxmox_virtual_environment_vm.k8s-ctrl[each.value.name].ipv4_addresses[1][0]
